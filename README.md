...
All architecture decisions and research are stored in `.docs/`:

| File | Content |
|---|---|
| `01-architecture.md` | System design and data flow |
| `02-tech-stack.md` | Tool choices and trade-offs |
| `03-offside-logic.md` | Offside rule + algorithm detail |
| `04-detection-model.md` | YOLO vs MediaPipe evaluation |
| `05-api-spec.md` | REST API spec (Phase 2) |
| `06-setup-guide.md` | Local dev setup |
| `07-deployment.md` | Production deployment checklist |
| `08-existing-vs-proposed-architecture.md` | Migration path from Gradio MVP |
| `09-BRD-alignment-and-updates.md` | Business requirements alignment |
| `10-kaggle-kernel-setup.md` | Step-by-step Kaggle setup |
| `11-kaggle-execution-summary.md` | Execution model and architecture |
...
- Every module has a `test_*.py` with deterministic fixtures
- All design decisions logged in `.docs/` as markdown
- UV for all dependency changes (`uv add`, `uv sync`)
- No destructive git commands (`reset --hard`, `push --force`) without explicit approval
...
Shakil Mosharrof - shakilmrf8@gmail.com
